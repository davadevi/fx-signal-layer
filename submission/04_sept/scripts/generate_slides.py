...
def slide_stages(prs):
    ...
    rows = [
        ...
        ["03 · Бэктест",     "✓ Завершён",   "Walk-forward 2y/3m, embargo 5d, hit def A+B, CI bootstrap",
         "KGS/TJS: lift 1.5–1.6, CI lower bound > 1.0"],
        ["04 · ML-слой",     "◑ Частично",   "LightGBM, асимметричный loss (FP × 3), two-tier порог",
         "Модель обучена; финальная OOT валидация — pending"],
        ["05 · Pipeline",    "◑ Частично",   "Mandatory/optional tier, cooldown 3d, compliance validator",
         "generate_signals() готов; интеграционные тесты — нет"],
    ]
    add_table(slide, headers, rows,
              x=0.5, y=1.45, w=12.3, h=5.7,
              col_widths=[2.0, 1.5, 4.8, 4.0])


def slide_results(prs):
    slide = blank_slide(prs)
    slide_header(slide, "Предварительные результаты",
                 "Индикатор: LogReturnPercentile · confirm_days=2 · h=5 рабочих дней")

    headers = ["Коридор", "Lift h=5", "CI 90% нижняя", "Сигн. / нед.", "Вердикт"]
    rows = [
        ["RUB / KGS (сом)",    "1.60", "1.07", "0.09", "✓ CI > 1.0"],
        ["RUB / TJS (сомони)", "1.48", "1.07", "0.11", "✓ CI > 1.0"],
        ["RUB / AMD (драм)",   "1.41", "0.80", "0.08", "⚠ CI < 1.0"],
        ["RUB / UZS (сум)",    "1.22", "0.76", "0.11", "⚠ CI < 1.0"],
        ["RUB / KZT (тенге)",  "1.01", "0.50", "0.10", "✗ Нет сигнала"],
    ]
    add_table(slide, headers, rows,
              x=0.5, y=1.45, w=8.5, h=4.2,
              col_widths=[2.5, 1.3, 1.8, 1.8, 1.9])

    # Callout: limitation
    box = slide.shapes.add_shape(
        1, Inches(9.3), Inches(1.45), Inches(3.5), Inches(4.2)
    )
    box.fill.solid()
    box.fill.fore_color.rgb = RED_BG
    box.line.color.rgb = RED
    ...
    lines = [
        ("Честное ограничение", {"size": 13, "bold": True, "color": RED}),
        ("", {}),
        ("Частота 0.08–0.11 сигн./нед значительно ниже целевых 1–2.", {"size": 11, "color": DARK}),
        ("", {}),
        ("Строгое подтверждение (confirm=2) обеспечивает точность, но режет частоту.", {"size": 11, "color": GRAY}),
        ("", {}),
        ("Trade-off: релакс порога повышает частоту за счёт снижения lift.", {"size": 11, "color": GRAY}),
        ("", {}),
        ("KGS и TJS — единственные коридоры с CI нижней границей > 1.0.", {"size": 11, "color": DARK}),
    ]
    first = True
    ...
    add_box(slide, 0.5, 5.8, 12.3, 0.45,
...
def slide_open_questions(prs):
    ...
    questions = [
        (
            "Частота vs точность",
            "Текущий confirm_days=2 обеспечивает lift 1.5–1.6 на KGS/TJS, но даёт 0.09 сигн./нед — в 10× ниже цели. "
            "Relaxed вариант (confirm_days=0) даёт 0.46–0.62 сигн./нед, но lift не верифицирован на OOT."
        ),
        (
            "Бэктест ≠ runtime",
            "Логика фильтрации в walk-forward engine и в generate_signals() реализована по-разному. "
            "Метрики бэктеста не воспроизводятся точно при запуске pipeline в продакшн-режиме."
        ),
        (
            "Экономический эффект в bps",
            "Hit rate > 50% на KGS и TJS подтверждён, но средний прирост в базисных пунктах на горизонте h=5 "
            "остаётся отрицательным — направление движения верное, но magnitude недостаточна."
        ),
    ]
...
def slide_plan(prs):
    slide = blank_slide(prs)
    slide_header(slide, "План работ до финальной защиты")

    phases = [
        (
            "4–5 сентября",
            [
                "Синхронизировать логику фильтрации: backtest ↔ generate_signals()",
                "OOT валидация с pathwise критерием (курс не хуже ни в один из T+1..T+h дней)",
                "Проверить weak-tier (confirm_days=0): частота и lift на нетронутых данных",
                "Зафиксировать решение по порогу; пересчитать bps на h=10/20",
            ]
        ),
        (
            "6 сентября",
            [
                "Интеграционные тесты end-to-end (test_end_to_end.py)",
                "REPRODUCE.md — точные команды для воспроизведения pipeline",
                "PILOT.md — дизайн пилота, A/B метрики, стоп-критерии",
                "LIMITATIONS.md — честные ограничения: pathwise, bps, частота",
            ]
        ),
        (
            "7 сентября · Финальная защита",
            [
                "Финальный отчёт с метриками по всем коридорам",
                "Пакет документации: README, REPRODUCE, PILOT, LIMITATIONS",
                "Демо pipeline на тестовых данных",
            ]
        ),
    ]

    colors = [RED_BG, LGRAY, RGBColor(0xE8, 0xF5, 0xE9)]
    y_positions = [1.45, 3.3, 5.25]

    for (phase_title, items), bg_color, y in zip(phases, colors, y_positions):
        box = slide.shapes.add_shape(
            1, Inches(0.5), Inches(y), Inches(12.3), Inches(1.65)
        )
        box.fill.solid()
